...
| `season-subtitle-cascade/episode-001.xml` | Same-season subtitle matching | *The Eminence in Shadow*, season 1 episode 1; HEVC and AV1 versions, Italian ASS embedded and external, different source stream IDs. |
| `season-subtitle-cascade/episode-002.xml` | Same-season subtitle matching | *The Eminence in Shadow*, season 1 episode 2; equivalent Italian tracks with different source stream IDs. |
| `season-subtitle-cascade/episode-003.xml` | Same-season subtitle matching | *The Eminence in Shadow*, season 1 episode 3; equivalent Italian tracks with different source stream IDs. |
| `subtitles/forced.xml` | Forced subtitles | English ASS track marked forced alongside normal subtitle tracks. |
| `subtitles/external-srt.xml` | External SRT | *The Eminence in Shadow*, season 2 episode 1; external Italian SRT with its Plex stream endpoint. |
...
| `multipart/multipart.xml` | Multi-part media | *The Quintessential Quintuplets∽* represented by two MKV `Part` nodes under one `Media`. |
| `multiversion/multiversion.xml` | Multi-version media | *The Eminence in Shadow*, episode 1 with HEVC/AAC and AV1/OPUS versions. |
| `extras/parent.xml` | Extras parent | *Demon Slayer: Il Castello dell'Infinito - Parte 1*. |
| `extras/extras.xml` | Extras collection | Two Plex extras/trailers for the parent item. |
| `hierarchy/series.xml` | Series hierarchy root | *The Eminence in Shadow* show metadata and season-level identity. |
...
| `collections/code-geass.xml` | Collection order | Four *Code Geass* collection members in the order returned by Plex. |
...
| `edge/no-subtitles.xml` | No subtitle alternatives | *Date A Live: Mayuri Judgment* with one video stream and one audio stream, and no subtitle streams. |

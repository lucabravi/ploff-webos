...
var modulePath = path.join(__dirname, '..', 'app', 'chapter-state.js');
...
assert.strictEqual(ChapterState.indexAt([
  { startTimeOffset: 0 }, { startTimeOffset: 764000 }, { startTimeOffset: 1260000 }
], 906000), 1, 'the current chapter must be the closest start at or before 15:06');
...
assert.deepStrictEqual(state, { open: true, index: 1, currentIndex: 1 }, 'opening the drawer must focus the current chapter');
...
assert.deepStrictEqual(selection.state, { open: false, index: 1, currentIndex: 1 }, 'selecting a chapter must close the drawer');
assert.deepStrictEqual(ChapterState.close(state), { open: false, index: 1, currentIndex: 1 }, 'Back or Up must close the chapter drawer without changing selection');
assert.deepStrictEqual(ChapterState.open(ChapterState.create(), [], 0), { open: false, index: -1, currentIndex: -1 }, 'media without chapters must keep the drawer closed');

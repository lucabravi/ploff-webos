...
assert.strictEqual(focusContext.counters.querySelector, 0, 'focus movement must use mounted-node and cached-card references without descendant queries');
assert.ok(focusContext.counters.layoutReads <= 4, 'catalog focus movement must retain the real visibility guard without adding layout reads beyond the established bound');

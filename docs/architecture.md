...
- `shell/` contains the TV interface, player, Plex API client, authentication,
  settings, server discovery, and remote-control navigation.
- `shell/i18n.js` is the small locale registry; `shell/locales/` contains one
...
`shell/config.js` contains neutral publishable defaults. Packaging explicitly

...
if (wasm) {
  sourcePath = path.join(__dirname, 'vendor-sources', 'subtitles-octopus-worker-wasm-4.1.0.full.js.gz');
  outputPath = path.join(root, 'app', 'vendor', 'subtitles-octopus-worker.js');
  expectedSourceSha256 = 'f776a035923214e8f7eb6442ff92f3afa09b15b2847db3bf9c6aa4843ec5038f';
}
...
replaceAssignment('self.setTrack',
  ...
  'self.PloffAssWarmRender=function(index,time,last){var start;var warmMs;var result;var blendStart;var blendMs;var pixels=0;var i;if(self.PloffAssWarmCancelled){postMessage({target:"ploff-ass-warm-step",index:index,cancelled:true,warmStepCount:self.PloffAssWarmStepCount||0,warmTotalMs:self.PloffAssWarmTotalMs||0,warmMaxMs:self.PloffAssWarmMaxMs||0,warmComplete:self.PloffAssWarmComplete===true});return}start=performance.now();var warmImage=self.octObj.renderImage(Number(time)||0,self.changed);warmMs=performance.now()-start;blendStart=performance.now();result=self.buildResult(warmImage);blendMs=performance.now()-blendStart;for(i=0;i<result[0].length;i+=1){pixels+=result[0][i].w*result[0][i].h}self.PloffAssWarmStepCount=(self.PloffAssWarmStepCount||0)+1;self.PloffAssWarmTotalMs=(self.PloffAssWarmTotalMs||0)+warmMs;self.PloffAssWarmMaxMs=Math.max(self.PloffAssWarmMaxMs||0,warmMs);self.PloffAssWarmLastMs=warmMs;if(last&&pixels>0){self.PloffAssWarmComplete=true;PloffAssTimingMark("warm-render-ready")}postMessage({target:"ploff-ass-warm-step",index:index,libassMs:warmMs,blendMs:blendMs,pixelCount:pixels,canvases:result[0],warmStepCount:self.PloffAssWarmStepCount,warmTotalMs:self.PloffAssWarmTotalMs,warmMaxMs:self.PloffAssWarmMaxMs,warmComplete:self.PloffAssWarmComplete===true},result[1])};' +
...
var externalized = wasm ? {
  output: 'var PloffAssTimingEnabled=true;function PloffAssTimingMark(phase){postMessage({target:"ploff-ass-timing",phase:phase})}PloffAssTimingMark("worker-script");' + profiledOutput,
  memory: Buffer.alloc(0), segmentCount: 0
} : externalizeStaticMemory(profiledOutput);
...
if (checkOnly) {
  ...
  if (!wasm && (!fs.existsSync(memoryOutputPath) || !fs.readFileSync(memoryOutputPath).equals(externalized.memory))) {
    throw new Error('Legacy ASS worker static memory is not synchronized; run npm run build:ass-legacy-worker');
  }
} else {
  fs.writeFileSync(outputPath, output);
  if (!wasm) { fs.writeFileSync(memoryOutputPath, externalized.memory); }
}
console.log((wasm ? 'WASM' : 'Legacy') + ' ASS worker' + (checkOnly ? ' check' : '') + ': ' + sourceBytes + ' -> ' + outputBytes + ' JS bytes (' +
